...
export function AppShell({ children }: { children: ReactNode }) {
    ...
    // B. User is authenticated, now check profile state
    if (userProfile === null) {
        // This is the critical error state where auth succeeded but profile fetch failed.
        return <ErrorShell 
            title="Error al cargar el perfil"
            description="Hemos detectado un problema al cargar los datos de tu perfil. Por favor, intenta cerrar sesión y volver a ingresar. Si el problema persiste, contacta a soporte." 
        />;
    }
    ...
    // C. User is fully authenticated and has a valid profile
    if (instituteId) {
         if (isPublicRoute) {
            // This is handled by useEffect, but it's a safe fallback.
            return <FullscreenLoader />;
         }
        return (
