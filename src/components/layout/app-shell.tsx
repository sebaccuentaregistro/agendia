...
export function AppShell({ children }: { children: ReactNode }) {
    ...
    if (isPublicRoute) {
        // If we are on a public route, we either show the page or the loader if we're still processing login
        if (!user) {
            return <>{children}</>;
        }
        // If user is logged in on a public page, show loader while we redirect
        return <FullscreenLoader />;
    }
    ...
    if (user && userProfile) {
        ...
        // If status is active but no instituteId, it's an error state
        if (userProfile.status === 'active' && !userProfile.instituteId) {
            return <ErrorShell 
    ...
    // Fallback for any other weird state (e.g. user exists but profile fetch failed)
    return <ErrorShell 
        title="Error al Cargar Perfil"
        description="No pudimos cargar los datos de tu perfil. Por favor, intenta cerrar sesión y volver a ingresar. Si el problema persiste, contacta a soporte."
    />;
}
